% % ==========================================================
% % function_Recon_RC.m (V7 - 修复版 + 动态进度条)
% % ==========================================================
function [volume_RC, x_axis, y_axis, z_axis] = function_Recon_RC(RcvData, Trans, Resource, TX, TW, Receive, angle_set)
% [!!] 注意：第一行必须是 function 定义！

% disp(' [Function] 正在执行: 行发列收 (RC) 重建...');
...
for n_wave = 1:length(TX), steer(n_wave,:) = TX(n_wave).Steer; end
...
fprintf('\n');
fprintf(' [Function] RC 完成, 耗时: %.2f s\n', toc);

% --- 5. 输出 ---
fk_data2 = ifft(ifft(fk_data,[],1),[],2);
volume_RC = fk_data2(140:140+1023, 1:128, :);
z_axis = linspace(5e-3, 42e-3, size(volume_RC,1)); 
x_axis = linspace(-127*pitch/2, 127*pitch/2, 128);    
y_axis = linspace(-127*pitch/2, 127*pitch/2, size(volume_RC,3));
end