...
% 
%     input/
%         F_RC_real              [Nz, Nx, Ny, 128, N_input_angle]
%         F_RC_imag              [Nz, Nx, Ny, 128, N_input_angle]
%         F_CR_real              [Nz, Nx, Ny, 128, N_input_angle]
%         F_CR_imag              [Nz, Nx, Ny, 128, N_input_angle]
% 
%     baseline/
%         DAS_input_sum_real     [Nz, Nx, Ny]
%         DAS_input_sum_imag     [Nz, Nx, Ny]
%         DAS_input_sum_abs      [Nz, Nx, Ny]
% 
%         DAS_input_avg_real     [Nz, Nx, Ny]
%         DAS_input_avg_imag     [Nz, Nx, Ny]
%         DAS_input_avg_abs      [Nz, Nx, Ny]
% 
%     label/
%         DAS_target_sum_real    [Nz, Nx, Ny]
%         DAS_target_sum_imag    [Nz, Nx, Ny]
%         DAS_target_sum_abs     [Nz, Nx, Ny]
% 
%         DAS_target_avg_real    [Nz, Nx, Ny]
%         DAS_target_avg_imag    [Nz, Nx, Ny]
%         DAS_target_avg_abs     [Nz, Nx, Ny]
% 
%     meta/
%         z_idx
%         x_idx
...
for i = 1 : length(fileList)
    ...
    inspect_RF_learning_sample_v2(sample);
    
    save_path = fullfile('G:\DAS\Sample\', 'Phantom_050_learning_sample_000001_v2.h5');
    ...
    inspect_RF_learning_sample_v2(loaded_sample);
...
function plot_das_pair_debug(DAS_input, DAS_target, x_mm, z_mm, iy_mid, title_input, title_target)
    ...
    sgtitle(sprintf('%s vs %s, y-slice %d', title_input, title_target, iy_mid));
end
...
function inspect_RF_learning_sample_v2(sample)
% ============================================================
% inspect_RF_learning_sample_v2
% ============================================================

    F_RC = sample.input.F_RC_patch;
    F_CR = sample.input.F_CR_patch;

    DAS_input_sum  = sample.baseline.DAS_input_sum;
    DAS_input_avg  = sample.baseline.DAS_input_avg;

    DAS_target_sum = sample.label.DAS_target_sum;
    DAS_target_avg = sample.label.DAS_target_avg;

    fprintf('\n=== inspect_RF_learning_sample_v2 ===\n');

    fprintf('Input F_RC size          : [%s]\n', num2str(size(F_RC)));
    fprintf('Input F_CR size          : [%s]\n', num2str(size(F_CR)));
    fprintf('Baseline DAS sum size    : [%s]\n', num2str(size(DAS_input_sum)));
    fprintf('Baseline DAS avg size    : [%s]\n', num2str(size(DAS_input_avg)));
    fprintf('Target DAS sum size      : [%s]\n', num2str(size(DAS_target_sum)));
    fprintf('Target DAS avg size      : [%s]\n', num2str(size(DAS_target_avg)));

    fprintf('\n[NaN / Inf check]\n');
    fprintf('F_RC NaN count           : %d\n', count_nan_complex(F_RC));
    fprintf('F_CR NaN count           : %d\n', count_nan_complex(F_CR));
    fprintf('DAS_input_sum NaN count  : %d\n', count_nan_complex(DAS_input_sum));
    fprintf('DAS_target_sum NaN count : %d\n', count_nan_complex(DAS_target_sum));
    fprintf('DAS_input_avg NaN count  : %d\n', count_nan_complex(DAS_input_avg));
    fprintf('DAS_target_avg NaN count : %d\n', count_nan_complex(DAS_target_avg));

    fprintf('F_RC Inf count           : %d\n', count_inf_complex(F_RC));
    fprintf('F_CR Inf count           : %d\n', count_inf_complex(F_CR));
    fprintf('DAS_input_sum Inf count  : %d\n', count_inf_complex(DAS_input_sum));
    fprintf('DAS_target_sum Inf count : %d\n', count_inf_complex(DAS_target_sum));
    fprintf('DAS_input_avg Inf count  : %d\n', count_inf_complex(DAS_input_avg));
    fprintf('DAS_target_avg Inf count : %d\n', count_inf_complex(DAS_target_avg));

    fprintf('\n[Amplitude statistics: RF input]\n');
    fprintf('|F_RC| min / mean / max = %.3e / %.3e / %.3e\n', ...
        min(abs(F_RC(:))), mean(abs(F_RC(:))), max(abs(F_RC(:))));
    fprintf('|F_CR| min / mean / max = %.3e / %.3e / %.3e\n', ...
        min(abs(F_CR(:))), mean(abs(F_CR(:))), max(abs(F_CR(:))));

    fprintf('\n[Amplitude statistics: DAS sum]\n');
    fprintf('|DAS input sum|  min / mean / max = %.3e / %.3e / %.3e\n', ...
        min(abs(DAS_input_sum(:))), mean(abs(DAS_input_sum(:))), max(abs(DAS_input_sum(:))));
    fprintf('|DAS target sum| min / mean / max = %.3e / %.3e / %.3e\n', ...
        min(abs(DAS_target_sum(:))), mean(abs(DAS_target_sum(:))), max(abs(DAS_target_sum(:))));
    fprintf('|target sum - input sum| mean / max = %.3e / %.3e\n', ...
        mean(abs(DAS_target_sum(:) - DAS_input_sum(:))), ...
        max(abs(DAS_target_sum(:) - DAS_input_sum(:))));

    fprintf('\n[Amplitude statistics: DAS angle-average]\n');
    fprintf('|DAS input avg|  min / mean / max = %.3e / %.3e / %.3e\n', ...
        min(abs(DAS_input_avg(:))), mean(abs(DAS_input_avg(:))), max(abs(DAS_input_avg(:))));
    fprintf('|DAS target avg| min / mean / max = %.3e / %.3e / %.3e\n', ...
        min(abs(DAS_target_avg(:))), mean(abs(DAS_target_avg(:))), max(abs(DAS_target_avg(:))));
    fprintf('|target avg - input avg| mean / max = %.3e / %.3e\n', ...
        mean(abs(DAS_target_avg(:) - DAS_input_avg(:))), ...
        max(abs(DAS_target_avg(:) - DAS_input_avg(:))));

    % ------------------------------------------------------------
    % Visualization: sum and average comparison
    % ------------------------------------------------------------
    scan_patch = sample.meta.scan_patch;
    ...
    iy_mid = ceil(scan_patch.N_y / 2);

    plot_das_pair_debug(DAS_input_sum, DAS_target_sum, x_mm, z_mm, iy_mid, ...
        '3-angle DAS sum', '33-angle DAS sum');

    plot_das_pair_debug(DAS_input_avg, DAS_target_avg, x_mm, z_mm, iy_mid, ...
        '3-angle DAS avg', '33-angle DAS avg');

    % Center voxel RF input map
    izc = ceil(scan_patch.N_z / 2);
    ixc = ceil(scan_patch.N_x / 2);
    iyc = ceil(scan_patch.N_y / 2);

    F_RC_center = squeeze(abs(F_RC(izc, ixc, iyc, :, :)));
    F_CR_center = squeeze(abs(F_CR(izc, ixc, iyc, :, :)));
