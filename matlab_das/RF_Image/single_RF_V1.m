...
for i = 1 : length(fileList)
    ...
    % ============================================================
    % V1: delay-aligned RF patch extraction
    % ============================================================
    
%     angle_set_debug = [3, 38, 73];
    idx_33 = round(linspace(1, 75, 33));
    angle_set_debug = idx_33;

    z_idx = 509:516;
    x_idx = 20:23;
    y_idx = 63:66;
    
%     test_patches(1).z_idx = 200:207;
%     test_patches(1).x_idx = 63:66;
%     test_patches(1).y_idx = 63:66;
    
%     test_patches(1).z_idx = 800:807;
%     test_patches(1).x_idx = 63:66;
%     test_patches(1).y_idx = 63:66;

    test_patches(1).z_idx = 505:520;
    test_patches(1).x_idx = 61:68;
    test_patches(1).y_idx = 61:68;

    pid = 1;
    
    z_idx = test_patches(pid).z_idx;
    x_idx = test_patches(pid).x_idx;
    y_idx = test_patches(pid).y_idx;
    

    
    % 1) 只提取 RF patch tensor
    out_patch = extract_delay_aligned_RF_patch_v1( ...
        RF_Single, D.Trans, Res, D.TX, D.TW, Rec_S, scan, ...
        angle_set_debug, z_idx, x_idx, y_idx);
    
    % 2) 单独验证：不是提取函数的一部分
    val_patch = validate_delay_aligned_RF_patch_v1( ...
        out_patch, RF_Single, D.Trans, Res, D.TX, D.TW, Rec_S);
    
    % 3) 单独画图：也不是提取函数的一部分
    plot_delay_aligned_patch_validation_v1(val_patch);
    

    % ============================================================
    % V2: save / load / validate one RF patch HDF5
    % ============================================================
    
    save_path = fullfile(pwd, 'Phantom_050_RFpatch_000001.h5');
    
    save_RF_patch_h5_v1(out_patch, save_path);
    
    loaded_patch = load_RF_patch_h5_v1(save_path);
    
    h5_val = validate_RF_patch_h5_v1(out_patch, loaded_patch);
