...
PROJECT_ROOT = next(
    ...
    if (parent / "rf_cached_dataset.py").exists() and (parent / "rf_cgan_losses.py").exists()
)
...
def cell1_self_check(config: dict[str, Any]) -> dict[str, Any]:
    ...
    from rf_cgan_losses import STRUCT_NORMALIZATION, STRUCT_NORMALIZATION_EPS
    ...
    assert float(config["loss"]["lambda_adv"]) == 1.0, "lambda_adv 必须为 1.0"
    assert float(config["loss"]["lambda_struct"]) == 1.0, "lambda_struct 必须为 1.0"
    assert float(config["loss"]["lambda_carrier"]) == 0.25, "lambda_carrier 必须为 0.25"
    assert bool(config["loss"]["carrier"]["enabled"]) is True, "carrier 必须开启"
    assert config["loss"]["carrier"]["normalization"]["mode"] == STRUCT_NORMALIZATION, "carrier 归一化模式与 struct 不一致"
    assert float(config["loss"]["carrier"]["normalization"]["eps"]) == STRUCT_NORMALIZATION_EPS, "carrier eps 与 struct 不一致"
    assert config["loss"]["struct_normalization"]["mode"] == STRUCT_NORMALIZATION, "struct 归一化模式与代码不一致"
    assert float(config["loss"]["struct_normalization"]["eps"]) == STRUCT_NORMALIZATION_EPS, "struct eps 与代码不一致"
    ...
    report = {
        ...
        "carrier": config["loss"]["carrier"],
        "struct_normalization": {
            "mode": STRUCT_NORMALIZATION,
            "eps": STRUCT_NORMALIZATION_EPS,
        },
...
def train_one_batch(
    ...
    struct_unnormalized_raw = finite_float(g_terms["g_struct_unnormalized_raw"])
    struct_scale = finite_float(g_terms["g_struct_scale"])
    carrier_raw = finite_float(g_terms["g_carrier_raw"])
    carrier_unnormalized_raw = finite_float(g_terms["g_carrier_unnormalized_raw"])
    carrier_scale = finite_float(g_terms["g_carrier_scale"])
    carrier_skipped = finite_float(g_terms["g_carrier_skipped"])
    metrics = {
        ...
        "G_struct_unnormalized_raw": struct_unnormalized_raw,
        "G_struct_scale": struct_scale,
        "G_carrier_raw": carrier_raw,
        "G_carrier_unnormalized_raw": carrier_unnormalized_raw,
        "G_carrier_scale": carrier_scale,
        ...
        "G_carrier_skipped": carrier_skipped,
...
@torch.no_grad()
def normalized_struct_self_check(
    config: dict[str, Any],
    data_state: dict[str, Any],
    models: dict[str, Any],
) -> dict[str, Any]:
    from rf_cgan_losses import generator_lsgan_struct_carrier_loss, random_y_index

    models["G"].eval()
    models["D"].eval()
    rng = random.Random(int(config["sampler"]["seed"]))
    batch = next(iter(data_state["train_loader"]))
    ...
    y_idx = random_y_index(label, rng=rng)
    g_loss, terms = generator_lsgan_struct_carrier_loss(
        models["D"],
        pred,
        label,
        baseline,
    ...
    struct_old = finite_float(terms["g_struct_unnormalized_raw"])
    struct_new = finite_float(terms["g_struct_raw"])
    scale = finite_float(terms["g_struct_scale"])
    expected = struct_old / scale
    adv_weighted = finite_float(terms["g_adv_weighted"])
    struct_weighted = finite_float(terms["g_struct_weighted"])
    carrier_raw = finite_float(terms["g_carrier_raw"])
    carrier_unnormalized_raw = finite_float(terms["g_carrier_unnormalized_raw"])
    carrier_scale = finite_float(terms["g_carrier_scale"])
    carrier_weighted = finite_float(terms["g_carrier_weighted"])
    carrier_skipped = finite_float(terms["g_carrier_skipped"])
    carrier_expected = carrier_unnormalized_raw / carrier_scale
    total_expected = adv_weighted + struct_weighted + carrier_weighted
    report = {
        "timestamp": timestamp(),
        "batch_categories": list(batch["category"]),
        "y_idx": int(y_idx),
        "scale_definition": config["loss"]["struct_normalization"],
        "struct_old_unnormalized": struct_old,
        "struct_new_normalized": struct_new,
        "struct_old_div_scale": expected,
        "abs_error_new_vs_old_div_scale": abs(struct_new - expected),
        "relative_error_new_vs_old_div_scale": abs(struct_new - expected) / max(abs(expected), 1e-12),
        "G_adv_weighted": adv_weighted,
        "G_struct_weighted": struct_weighted,
        "carrier_old_unnormalized": carrier_unnormalized_raw,
        "carrier_new_normalized": carrier_raw,
        "carrier_old_div_scale": carrier_expected,
        "carrier_scale": carrier_scale,
        "abs_error_carrier_new_vs_old_div_scale": abs(carrier_raw - carrier_expected),
        "relative_error_carrier_new_vs_old_div_scale": abs(carrier_raw - carrier_expected) / max(abs(carrier_expected), 1e-12),
        "G_carrier_weighted": carrier_weighted,
        "G_carrier_skipped": carrier_skipped,
        "G_total": finite_float(g_loss),
        "G_total_expected_adv_struct_carrier": total_expected,
        "G_total_abs_error_vs_components": abs(finite_float(g_loss) - total_expected),
        "pred_max_abs": float(pred.detach().float().abs().max().item()),
        "has_nan": bool(any(torch.isnan(t.detach().float()).any().item() for t in [pred, g_loss])),
        "has_inf": bool(any(torch.isinf(t.detach().float()).any().item() for t in [pred, g_loss])),
    }
    assert report["struct_new_normalized"] < 10.0, f"struct_new 未降到 O(1): {report}"
    assert report["G_adv_weighted"] < 10.0, f"adv 未在 O(1): {report}"
    assert report["G_struct_weighted"] < 10.0, f"struct 未在 O(1): {report}"
    assert report["G_carrier_weighted"] < 10.0, f"carrier 未在 O(1): {report}"
    assert report["G_carrier_skipped"] == 0.0, f"carrier 分支不应跳过: {report}"
    assert abs(report["carrier_scale"] - scale) / max(abs(scale), 1e-12) < 1e-7, f"carrier 与 struct 未使用同一 scale: {report}"
    assert report["relative_error_carrier_new_vs_old_div_scale"] < 1e-5, f"carrier 等价关系不成立: {report}"
    assert report["G_total_abs_error_vs_components"] < 1e-5, f"G_total 不是 adv+struct+carrier: {report}"
    assert report["relative_error_new_vs_old_div_scale"] < 1e-5, f"struct 等价关系不成立: {report}"
    assert not report["has_nan"], f"自检出现 NaN: {report}"
    assert not report["has_inf"], f"自检出现 Inf: {report}"
    (LOG_DIR / "struct归一化自检.json").write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")
    print(
        "STRUCT_NORMALIZATION_SELF_CHECK "
        f"old={struct_old:.6g} scale={scale:.6g} new={struct_new:.6g} "
        f"old/scale={expected:.6g} rel_err={report['relative_error_new_vs_old_div_scale']:.3g} "
        f"adv_w={adv_weighted:.6g} struct_w={struct_weighted:.6g} "
        f"carrier_old={carrier_unnormalized_raw:.6g} carrier_new={carrier_raw:.6g} "
        f"carrier_old/scale={carrier_expected:.6g} carrier_w={carrier_weighted:.6g} "
        f"carrier_skipped={carrier_skipped:.0f} "
        f"total={report['G_total']:.6g}",
        flush=True,
    )
    return report
...
def aggregate_epoch(epoch: int, batch_metrics: list[dict[str, float]], seconds: float, peak_gb: float) -> dict[str, Any]:
    mean_keys = [
        ...
        "G_adv_raw",
        "G_struct_raw",
        "G_struct_unnormalized_raw",
        "G_struct_scale",
        "G_carrier_raw",
        "G_carrier_unnormalized_raw",
        "G_carrier_scale",
        "G_adv_weighted",
        "G_struct_weighted",
        "G_carrier_weighted",
...
def cell5_train_loop(
    ...
    rng = random.Random(int(config["sampler"]["seed"]))
    normalized_struct_self_check(config, data_state, models)
