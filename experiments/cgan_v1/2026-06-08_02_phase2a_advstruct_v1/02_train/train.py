...
def cell1_self_check(config: dict[str, Any]) -> dict[str, Any]:
    ...
    assert float(config["loss"]["lambda_carrier"]) == 0.0, "lambda_carrier 必须为 0.0"
    assert bool(config["loss"]["carrier"]["enabled"]) is False, "carrier 必须关闭"
...
def normalized_struct_self_check(
    ...
    total_expected = adv_weighted + struct_weighted
    report = {
        "timestamp": timestamp(),
        "batch_categories": list(batch["category"]),
        "y_idx": int(y_idx),
        ...
        "G_carrier_skipped": carrier_skipped,
        "G_total": finite_float(g_loss),
        "G_total_expected_adv_plus_struct": total_expected,
        "G_total_abs_error_vs_adv_plus_struct": abs(finite_float(g_loss) - total_expected),
        "pred_max_abs": float(pred.detach().float().abs().max().item()),
    ...
    assert report["G_carrier_weighted"] == 0.0, f"carrier_weighted 必须为 0: {report}"
    assert report["G_carrier_skipped"] == 1.0, f"carrier 分支未跳过: {report}"
    assert report["G_total_abs_error_vs_adv_plus_struct"] < 1e-5, f"G_total 不是 adv+struct: {report}"
    ...
    print(
        ...
        f"carrier_w={carrier_weighted:.6g} carrier_skipped={carrier_skipped:.0f} "
