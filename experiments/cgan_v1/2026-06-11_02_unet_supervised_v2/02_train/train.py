...
def make_model_state(config: dict[str, Any]) -> dict[str, Any]:
    add_import_paths()
    from rf_cgan_models import Light3DUNet
...
def cell1_self_check(config: dict[str, Any]) -> dict[str, Any]:
    ...
    y6 = state["G"](x6)
    batch6_loss = y6.mean()
    assert torch.isfinite(batch6_loss).item(), "batch=6 loss is not finite"
...
def cell3_data(config: dict[str, Any]) -> dict[str, Any]:
    ...
    train_fixed_indices = pick_first_per_category(datasets["train"], config["data"]["categories"])
    ...
    return {
        ...
        "train_fixed_indices": train_fixed_indices,
...
def cell5_train(config: dict[str, Any], data_state: dict[str, Any], model_state: dict[str, Any]) -> dict[str, Any]:
    ...
    for epoch in range(1, epochs + 1):
        ...
        generate_epoch_probes(
            ...
            train_indices=data_state["train_fixed_indices"],
            val_indices=data_state["fixed_indices"],
...
def cell6_loss_curves() -> dict[str, Any]:
    ...
    save_loss_curves(rows)
    return {"loss_curve": str(FIG_DIR / "light3dunet_bmode_supervised_loss_curves.png"), "rows": len(rows)}
