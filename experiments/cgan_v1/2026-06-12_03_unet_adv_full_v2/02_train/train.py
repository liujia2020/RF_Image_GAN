...
def check_redlines(row: dict[str, Any], config: dict[str, Any]) -> None:
    ...
    if epoch >= int(adv_cfg["min_epoch_for_flat_d"]):
        ...
        real_key = "d_real_score_sigmoid_train" if "d_real_score_sigmoid_train" in row else "d_real_score_sigmoid"
        fake_key = "d_fake_score_sigmoid_train" if "d_fake_score_sigmoid_train" in row else "d_fake_score_sigmoid"
        real_flat = abs(float(row[real_key]) - 0.5) <= eps
        fake_flat = abs(float(row[fake_key]) - 0.5) <= eps
        if real_flat and fake_flat:
            raise RuntimeError(
                f"REDLINE: train-mode D_real/D_fake sigmoid both near 0.5 at epoch {epoch}: "
