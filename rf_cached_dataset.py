...
class RFCachedDataset(Dataset):
    ...
    def __init__(self, cache_dir, return_fp16: bool = False, restore_scale: bool = True):
    ...
    def _read_cached_tensor(self, mmap, idx, scale: float) -> torch.Tensor:
        if self.restore_scale and self.normalized:
            # Restore in fp32. Returning fp16 after this can overflow for true RF
            # amplitudes above 65504, so restored tensors are always fp32.
        ...
        return torch.from_numpy(np.array(mmap[idx], dtype=out_dtype, copy=True))

    def __getitem__(self, idx):
        scale = float(self.scale[idx])
        return {
            "input": self._read_cached_tensor(self.input, idx, scale),
            "label": self._read_cached_tensor(self.label, idx, scale),
            "baseline": self._read_cached_tensor(self.baseline, idx, scale),
