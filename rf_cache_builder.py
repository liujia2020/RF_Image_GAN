...
def parse_args() -> argparse.Namespace:
    ...
    normalize_group.add_argument(
        ...
        default=True,
        help=(
            "Scale input/label/baseline by the per-sample label max before "
            "writing fp16 cache. Enabled by default."
        ),
    )
    normalize_group.add_argument(
        "--no-normalize",
        dest="normalize",
        action="store_false",
        help="Disable per-sample scaling before writing fp16 cache.",
    )
