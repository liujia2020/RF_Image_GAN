...
% Safety switch. This z-band script writes only temporary verification patches.
run_generation = true;
run_grid_dry_run = false;

if ~run_generation && run_grid_dry_run
    dry_run_patch_size = [64, 32, 32];
    dry_run_stride = [64, 32, 32];
    dry_run_z_range = [];

    scan = build_default_RCA_scan_v1();
    patch_list = make_patch_index_list_dense_v1( ...
        scan, dry_run_patch_size, dry_run_stride, dry_run_z_range);

    z_starts = arrayfun(@(p) p.z_idx(1), patch_list);
    x_starts = arrayfun(@(p) p.x_idx(1), patch_list);
    y_starts = arrayfun(@(p) p.y_idx(1), patch_list);

    first = patch_list(1);
    last = patch_list(end);

    fprintf('\nDense 64x32x32 no-overlap full-volume grid dry run\n');
    fprintf('  patch_size       : %s\n', mat2str(dry_run_patch_size));
    fprintf('  stride           : %s\n', mat2str(dry_run_stride));
    fprintf('  z_range          : %s\n', mat2str(dry_run_z_range));
    fprintf('  total patches    : %d\n', numel(patch_list));
    fprintf('  z windows        : %d\n', numel(unique(z_starts)));
    fprintf('  x windows        : %d\n', numel(unique(x_starts)));
    fprintf('  y windows        : %d\n', numel(unique(y_starts)));
    fprintf('  first z_idx      : %d ~ %d\n', first.z_idx(1), first.z_idx(end));
    fprintf('  first x_idx      : %d ~ %d\n', first.x_idx(1), first.x_idx(end));
    fprintf('  first y_idx      : %d ~ %d\n', first.y_idx(1), first.y_idx(end));
    fprintf('  last z_idx       : %d ~ %d\n', last.z_idx(1), last.z_idx(end));
    fprintf('  last x_idx       : %d ~ %d\n', last.x_idx(1), last.x_idx(end));
    fprintf('  last y_idx       : %d ~ %d\n', last.y_idx(1), last.y_idx(end));
    return;
end
...
OutputRoot = 'F:\DAS\RF_LearningSamples_dense_RF000489_zband448-576_halfstride';
DenseManifestCsv = fullfile(ManifestDir, ...
    'RF_manifest_dense_RF000489_zband448-576_halfstride.csv');
...
if ~exist(selected_file_path, 'file')
    remapped_file_path = strrep(selected_file_path, 'G:\Data_0110_RFdata', DataRoot);

    if exist(remapped_file_path, 'file')
        selected_file_path = remapped_file_path;
        dense_manifest.file_path = {selected_file_path};
    else
        error('Selected RF file does not exist: %s', selected_file_path);
    end
end
...
gen_opts.stride = stride;
gen_opts.z_range = z_range;
...
fprintf('  expected files   : see generated patch grid log\n');
