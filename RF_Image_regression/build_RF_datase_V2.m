...
% OutputRoot = 'G:\DAS\RF_LearningSamples_v2';
% 
% ManifestDir = fullfile(DataRoot, '_manifest');
% pilot_csv = fullfile(ManifestDir, 'RF_manifest_pilot_1perclass.csv');
...
manifest = build_RF_dataset_manifest_v2_simple(DataRoot, ManifestDir);

pilot_manifest = make_pilot_manifest_v1( ...
    manifest, 1, fullfile(ManifestDir, 'RF_manifest_pilot_1perclass_v2_simple.csv'));
pilot_csv = fullfile(ManifestDir, 'RF_manifest_pilot_1perclass_v2_simple.csv');


input_angle_set  = [3, 38, 73];
target_angle_set = round(linspace(1, 75, 33));

patch_size = [16, 8, 8];   % [Nz, Nx, Ny]
...
audit_table = audit_RF_learning_samples_folder_v1(OutputRoot);

% 保存审计结果
AuditDir = fullfile(OutputRoot, '_audit');
if ~exist(AuditDir, 'dir'), mkdir(AuditDir); end

writetable(audit_table, fullfile(AuditDir, 'pilot_audit_table.csv'));
save(fullfile(AuditDir, 'pilot_audit_table.mat'), 'audit_table');


%%
pilot10_manifest = make_pilot_manifest_v1( ...
    manifest, 10, fullfile(ManifestDir, 'RF_manifest_pilot_10perclass_v2_simple.csv'));

%% 说明四类数据的 .mat 变量结构都能被当前 loader 处理

pilot10_csv = fullfile(ManifestDir, 'RF_manifest_pilot_10perclass_v2_simple.csv');

gen_opts = struct();
gen_opts.overwrite = true;
gen_opts.validate_after_save = true;
gen_opts.max_files = inf;

log_table = generate_RF_learning_samples_from_manifest_v1( ...
    pilot10_csv, OutputRoot, ...
    input_angle_set, target_angle_set, ...
    patch_size, gen_opts);
%%
audit_table = audit_RF_learning_samples_folder_v1(OutputRoot);
...
full_manifest_csv = fullfile(ManifestDir, 'RF_manifest_balanced_v2_simple.csv');
manifest = readtable(full_manifest_csv);

filelevel_pilot_csv = fullfile(ManifestDir, ...
    'RF_manifest_filelevel_pilot_10train_2val_2test.csv');

pilot_manifest = make_filelevel_pilot_manifest_v1( ...
    manifest, ...
    10, 2, 2, ...
    filelevel_pilot_csv);

OutputRoot = 'G:\DAS\RF_LearningSamples_filelevel_pilot';
...
log_table = generate_RF_learning_samples_from_manifest_v1( ...
    filelevel_pilot_csv, OutputRoot, ...
    input_angle_set, target_angle_set, ...
    patch_size, gen_opts);
...
writetable(audit_table, fullfile(AuditDir, 'filelevel_pilot_audit_table.csv'));
save(fullfile(AuditDir, 'filelevel_pilot_audit_table.mat'), 'audit_table');
%%
%% 2000
clc;

% ============================================================
% Full balanced file-level RF learning dataset generation
% ============================================================
...
% ------------------------------------------------------------
% Build full file-level manifest:
% each category:
%   train 70 files
%   val   15 files
%   test  15 files
...
filelevel_full_csv = fullfile(ManifestDir, ...
    'RF_manifest_filelevel_full_70train_15val_15test.csv');

full_filelevel_manifest = make_filelevel_pilot_manifest_v1( ...
    manifest, ...
    70, 15, 15, ...
    filelevel_full_csv);
...
OutputRoot = 'G:\DAS\RF_LearningSamples_filelevel_full_2000';

% ------------------------------------------------------------
% Learning sample settings
% ------------------------------------------------------------
...
% 建议 full dataset 第一次生成时用 false，防止中断后重跑覆盖已完成文件
gen_opts.overwrite = false;

% 之前 pilot 已经验证过保存/读回链路。
% full dataset 为了节省时间，可以先设 false。
% 如果你想更稳，也可以改成 true。
...
% ------------------------------------------------------------
% Generate HDF5 learning samples
% ------------------------------------------------------------

log_table = generate_RF_learning_samples_from_manifest_v1( ...
    filelevel_full_csv, OutputRoot, ...
    input_angle_set, target_angle_set, ...
    patch_size, gen_opts);

% ------------------------------------------------------------
% Audit generated samples
% ------------------------------------------------------------
...
writetable(audit_table, fullfile(AuditDir, 'filelevel_full_2000_audit_table.csv'));
save(fullfile(AuditDir, 'filelevel_full_2000_audit_table.mat'), 'audit_table');

fprintf('\nFull file-level dataset generation and audit finished.\n');

%% Nonpoint large patch dataset: 32 x 16 x 16
clc;

DataRoot = 'G:\Data_0110_RFdata';
ManifestDir = fullfile(DataRoot, '_manifest');

full_manifest_csv = fullfile(ManifestDir, ...
    'RF_manifest_filelevel_full_70train_15val_15test.csv');

manifest = readtable(full_manifest_csv);

% ------------------------------------------------------------
% Only use non-point categories
% ------------------------------------------------------------

keep = ...
    (strcmp(string(manifest.category), 'carotid') | ...
     strcmp(string(manifest.category), 'muscle')  | ...
     strcmp(string(manifest.category), 'phantom')) & ...
     logical(manifest.use_for_learning);

large_manifest = manifest(keep, :);

% Keep 5 patches per RF file.
% This gives:
% train: 3 × 70 × 5 = 1050
% val  : 3 × 15 × 5 = 225
% test : 3 × 15 × 5 = 225
large_manifest.num_patches(:) = 5;

large_manifest_csv = fullfile(ManifestDir, ...
    'RF_manifest_nonpoint_32x16x16_70train_15val_15test.csv');

writetable(large_manifest, large_manifest_csv);

fprintf('\nLarge nonpoint manifest saved:\n%s\n', large_manifest_csv);
fprintf('Selected RF files : %d\n', height(large_manifest));
fprintf('Planned samples   : %d\n', sum(large_manifest.num_patches));

% ------------------------------------------------------------
% Output folder
% ------------------------------------------------------------

OutputRoot = 'G:\DAS\RF_LearningSamples_nonpoint_32x16x16';
...
% 第一次生成建议 false。中断后可以接着跑，不覆盖已经生成的。
gen_opts.overwrite = false;

% 之前链路已经验证过。这里为了速度，先 false。
gen_opts.validate_after_save = false;

gen_opts.overwrite = false;
gen_opts.validate_after_save = false;
gen_opts.patch_grid_mode = 'sparse';  % 'sparse' = original v1, 'dense' = full tiling
gen_opts.max_files = inf;
...
writetable(audit_table, fullfile(AuditDir, 'nonpoint_32x16x16_audit_table.csv'));
save(fullfile(AuditDir, 'nonpoint_32x16x16_audit_table.mat'), 'audit_table');

fprintf('\nNonpoint large patch dataset generation and audit finished.\n');
%% Random64 pilot: one random 64 x 32 x 32 patch per nonpoint RF file
% Keep this block disabled until the random patch plan is confirmed.
run_random64_pilot80 = false;

if run_random64_pilot80
    clc;

    ProjectRoot = fileparts(mfilename('fullpath'));
    DataRoot = 'F:\Data_0110_RFdata';
    OldDataRoot = 'G:\Data_0110_RFdata';
    ManifestDir = fullfile(DataRoot, '_manifest');
    ...
    % If the manifest was created when data lived on G:, remap paths to DataRoot.
    manifest.file_path = cellstr(strrep(string(manifest.file_path), OldDataRoot, DataRoot));
    ...
    nonpoint_manifest = manifest(keep, :);

    % 19/4/4 per category gives 3 * (19 + 4 + 4) = 81 RF files.
    random64_manifest = make_filelevel_pilot_manifest_v1( ...
        nonpoint_manifest, 19, 4, 4, '');
    random64_manifest.num_patches(:) = 1;

    random64_manifest_csv = fullfile(ManifestDir, ...
        'RF_manifest_random64_pilot81_nonpoint_19train_4val_4test.csv');

    writetable(random64_manifest, random64_manifest_csv);

    fprintf('\nRandom64 pilot manifest saved:\n%s\n', random64_manifest_csv);
    fprintf('Selected RF files : %d\n', height(random64_manifest));
    fprintf('Planned samples   : %d\n', sum(random64_manifest.num_patches));

    OutputRoot = fullfile(ProjectRoot, 'RF_LearningSamples_random64_pilot80');
    ...
    gen_opts.overwrite = false;
    gen_opts.validate_after_save = false;
    gen_opts.patch_grid_mode = 'random_single';
    gen_opts.random_seed = 640032;
    gen_opts.max_files = inf;

    log_table = generate_RF_learning_samples_from_manifest_v1( ...
        random64_manifest_csv, OutputRoot, ...
        input_angle_set, target_angle_set, ...
        patch_size, gen_opts);
end
%% Dense tiled test volume: one carotid test RF file, 32 x 16 x 16
% Keep this block disabled until the dense patch plan is confirmed.
run_dense_carotid_test = false;

if run_dense_carotid_test
    clc;
    ...
    dense_test_file_id = '';  % Empty uses the first carotid test RF file.
    ...
    % Dense mode ignores manifest.num_patches and uses the actual tiled patch_list length.

    dense_manifest_csv = fullfile(ManifestDir, ...
        'RF_manifest_dense_carotid_test_32x16x16_onefile.csv');

    writetable(dense_manifest, dense_manifest_csv);

    OutputRoot = 'G:\DAS\RF_LearningSamples_dense_carotid_test_32x16x16';
...
function patch_list = make_patch_index_list_dense_v1(scan, patch_size, stride, z_range)
% ============================================================
% make_patch_index_list_dense_v1
%
% Generate a dense tiled patch list with optional overlap and z-band limit.
%
% patch_size = [Nz_patch, Nx_patch, Ny_patch]
% stride     = [Sz, Sx, Sy], defaults to patch_size
% z_range    = [z_min, z_max], defaults to full z range
...
% ============================================================
    ...
    if numel(stride) ~= 3
        error('stride must be [Sz, Sx, Sy].');
    end

    if any(stride <= 0) || any(mod(stride, 1) ~= 0)
        error('stride values must be positive integers.');
    end
    ...
    if nargin < 4 || isempty(z_range)
        z_range = [1, scan.N_z];
    end

    z_range = double(z_range(:)).';

    if numel(z_range) ~= 2
        error('z_range must be [z_min, z_max].');
    end

    if any(mod(z_range, 1) ~= 0)
        error('z_range values must be integer indices.');
    end

    z_min = z_range(1);
    z_max = z_range(2);

    if z_min < 1 || z_max > scan.N_z || z_min > z_max
        error('z_range must satisfy 1 <= z_min <= z_max <= scan.N_z.');
    end

    if (z_max - z_min + 1) < Nzp
        error('z_range length must be at least Nz_patch.');
    end

    z_last_start = z_max - Nzp + 1;
    x_last_start = scan.N_x - Nxp + 1;
    y_last_start = scan.N_y - Nyp + 1;

    z_starts_raw = z_min:Sz:z_max;
    x_starts_raw = 1:Sx:scan.N_x;
    y_starts_raw = 1:Sy:scan.N_y;

    z_starts = unique(min(z_starts_raw, z_last_start), 'stable');
    x_starts = unique(min(x_starts_raw, x_last_start), 'stable');
    y_starts = unique(min(y_starts_raw, y_last_start), 'stable');
...
function patch_list = make_patch_index_list_random_v1(scan, patch_size, random_seed)
% ============================================================
% make_patch_index_list_random_v1
%
% Generate one random in-bounds patch.
%
% patch_size  = [Nz_patch, Nx_patch, Ny_patch]
% random_seed = scalar seed for reproducible 1-based starts
%
% The returned z_idx/x_idx/y_idx are MATLAB 1-based indices, matching
% make_centered_index_v1 and the existing HDF5 meta convention.
% ============================================================
    ...
    max_start = scan_size - patch_size + 1;

    if nargin >= 3 && ~isempty(random_seed)
        old_rng = rng;
        cleanup_rng = onCleanup(@() rng(old_rng));
        rng(double(random_seed), 'twister');
    end

    z0 = randi([1, max_start(1)]);
    x0 = randi([1, max_start(2)]);
    y0 = randi([1, max_start(3)]);

    patch_list = struct('z_idx', {}, 'x_idx', {}, 'y_idx', {});
    patch_list(1).z_idx = z0:(z0 + Nzp - 1);
    patch_list(1).x_idx = x0:(x0 + Nxp - 1);
    patch_list(1).y_idx = y0:(y0 + Nyp - 1);

    fprintf('[Random patch] z_idx=%d~%d, x_idx=%d~%d, y_idx=%d~%d\n', ...
        patch_list(1).z_idx(1), patch_list(1).z_idx(end), ...
        patch_list(1).x_idx(1), patch_list(1).x_idx(end), ...
        patch_list(1).y_idx(1), patch_list(1).y_idx(end));
end
...
% ============================================================
    ...
    if ~isfield(gen_opts, 'stride') || isempty(gen_opts.stride)
        gen_opts.stride = patch_size;
    end

    if ~isfield(gen_opts, 'z_range')
        gen_opts.z_range = [];
    end

    if ~isfield(gen_opts, 'random_seed') || isempty(gen_opts.random_seed)
        gen_opts.random_seed = 1;
    end
    ...
    fprintf('Stride           : %s\n', mat2str(gen_opts.stride));
    fprintf('Random seed base : %d\n', gen_opts.random_seed);
    if isempty(gen_opts.z_range)
        fprintf('Z range          : full scan\n');
    else
        fprintf('Z range          : %s\n', mat2str(gen_opts.z_range));
    end
    fprintf('============================================================\n');
        ...
        try
            ...
            switch patch_grid_mode
                ...
                case {'dense', 'dense_v1', 'tile', 'tiled'}
                    patch_list = make_patch_index_list_dense_v1( ...
                        scan, patch_size, gen_opts.stride, gen_opts.z_range);
                ...
                case {'random_single', 'single_random', 'random_v1'}
                    random_seed_i = gen_opts.random_seed + row_idx - 1;
                    patch_list = make_patch_index_list_random_v1(scan, patch_size, random_seed_i);

                    if isfinite(num_patches) && num_patches ~= length(patch_list)
                        fprintf('[Info] random_single ignores manifest num_patches=%g; actual patches=%d.\n', ...
                            num_patches, length(patch_list));
                    end

                    fprintf('[Info] random_single seed=%d.\n', random_seed_i);
