clc; clear; close all;
% ============================================================
% Random64 full RF learning sample generation
%
% Purpose:
%   Generate the full nonpoint dataset for 64 x 32 x 32 patches.
%
% Selection:
%   Categories : carotid, muscle, phantom
%   Per category: train/val/test = 70/15/15 RF files
%   Total files : 3 * (70 + 15 + 15) = 300
%   Patches     : 5 random patches per RF file
%   Total samples: 1500 = train 1050 / val 225 / test 225
%
% Important:
%   The split is file-level. All 5 patches from one RF file stay in the
%   same split to avoid leakage.
%
% Patch:
%   patch_size = [64, 32, 32] = [Nz, Nx, Ny]
%   z_idx/x_idx/y_idx are MATLAB 1-based in HDF5 meta.
% ============================================================

% Safety switches. Dry-run prints manifest and random starts only.
...
random64_manifest = make_filelevel_pilot_manifest_v1( ...
    nonpoint_manifest, 70, 15, 15, '');
random64_manifest.num_patches(:) = 5;

random64_manifest_csv = fullfile(ManifestDir, ...
    'RF_manifest_random64_full1500_nonpoint_70train_15val_15test_5patch.csv');
writetable(random64_manifest, random64_manifest_csv);

OutputRoot = 'F:\DAS\RF_LearningSamples_random64_full1500';
...
fprintf('\nRandom64 full1500 settings:\n');
fprintf('  OutputRoot : %s\n', OutputRoot);
fprintf('  Manifest   : %s\n', random64_manifest_csv);
fprintf('  Files      : %d\n', height(random64_manifest));
fprintf('  Patches/file: %d\n', random64_manifest.num_patches(1));
fprintf('  Patch size : [%d %d %d]\n', patch_size(1), patch_size(2), patch_size(3));
fprintf('  Seed base  : %d\n', gen_opts.random_seed);

if run_dry_run
    run_random64_full1500_dry_run_v1(random64_manifest, patch_size, gen_opts);
    return;
end

if ~run_generation
    fprintf('Random64 full1500 generation is disabled. Set run_generation = true to run.\n');
    return;
end
...
function run_random64_full1500_dry_run_v1(manifest, patch_size, gen_opts)
% ============================================================
% run_random64_full1500_dry_run_v1
%
% Print split/category counts and sample random starts without generating HDF5.
% ============================================================
    ...
    fprintf('\n============================================================\n');
    fprintf('Random64 full1500 dry run\n');
    fprintf('Patch size       : %s\n', mat2str(patch_size));
    fprintf('Patch grid mode  : %s\n', gen_opts.patch_grid_mode);
    ...
    for s = 1:numel(splits)
        split_name = splits{s};
        idx_split = strcmp(string(manifest.split), split_name) & logical(manifest.use_for_learning);

        n_files = sum(idx_split);
        n_patches = sum(manifest.num_patches(idx_split));
        total_files = total_files + n_files;
        total_patches = total_patches + n_patches;

        fprintf('\nSplit: %s\n', split_name);
        fprintf('  files total   : %d\n', n_files);
        fprintf('  patches total : %d\n', n_patches);

        for c = 1:numel(categories)
            cat_name = categories{c};
            idx_cat = idx_split & strcmp(string(manifest.category), cat_name);
            fprintf('  %-8s files=%3d patches=%4d\n', ...
                cat_name, sum(idx_cat), sum(manifest.num_patches(idx_cat)));
        end
    end

    fprintf('\nGrand total files   : %d\n', total_files);
    fprintf('Grand total patches : %d\n', total_patches);

    file_ids = string(manifest.file_id);
    unique_file_ids = unique(file_ids, 'stable');

    for i = 1:numel(unique_file_ids)
        idx_file = file_ids == unique_file_ids(i);
        splits_this_file = unique(string(manifest.split(idx_file)));
        if numel(splits_this_file) ~= 1
            error('Data leakage risk: file_id=%s appears in multiple splits: %s', ...
                unique_file_ids(i), strjoin(splits_this_file, ','));
        end
    end

    fprintf('File-level split leakage check: PASS\n');

    probe_rows = find( ...
        strcmp(string(manifest.split), 'train') & ...
        strcmp(string(manifest.category), 'carotid') & ...
        logical(manifest.use_for_learning), 2, 'first');

    if numel(probe_rows) < 2
        probe_rows = find(logical(manifest.use_for_learning), 2, 'first');
    end

    all_probe_starts = cell(numel(probe_rows), 1);

    fprintf('\nProbe random starts for two RF files:\n');

    for k = 1:numel(probe_rows)
        row_idx = probe_rows(k);
        ...
        n_patches = manifest.num_patches(row_idx);
        random_seed_i = gen_opts.random_seed + row_idx - 1;

        patch_list = make_patch_index_list_random_v1( ...
            scan, patch_size, random_seed_i, n_patches);

        starts = zeros(numel(patch_list), 3);
        for p = 1:numel(patch_list)
            starts(p, :) = [ ...
                patch_list(p).z_idx(1), ...
                patch_list(p).x_idx(1), ...
                patch_list(p).y_idx(1)];
        end

        n_unique = size(unique(starts, 'rows'), 1);
        if n_unique ~= numel(patch_list)
            error('Random starts are not unique within file_id=%s.', file_id);
        end

        all_probe_starts{k} = starts;
        ...
        fprintf('  seed       : %d\n', random_seed_i);
        fprintf('  patches    : %d\n', numel(patch_list));
        fprintf('  unique starts within file: %d / %d\n', n_unique, numel(patch_list));

        for p = 1:numel(patch_list)
            fprintf('    patch %03d start=[%4d %3d %3d] z=%4d~%4d x=%3d~%3d y=%3d~%3d\n', ...
                p, starts(p, 1), starts(p, 2), starts(p, 3), ...
                patch_list(p).z_idx(1), patch_list(p).z_idx(end), ...
                patch_list(p).x_idx(1), patch_list(p).x_idx(end), ...
                patch_list(p).y_idx(1), patch_list(p).y_idx(end));
        end
    end

    if numel(all_probe_starts) == 2
        common_starts = intersect(all_probe_starts{1}, all_probe_starts{2}, 'rows');
        fprintf('\nProbe file-to-file identical start count: %d\n', size(common_starts, 1));

        if ~isempty(common_starts)
            error('Probe files unexpectedly share random start(s).');
        end
    end

    fprintf('\nDry run finished. No HDF5 files were generated.\n');
end
...
function patch_list = make_patch_index_list_random_v1(scan, patch_size, random_seed, num_patches)
% ============================================================
% make_patch_index_list_random_v1
%
% Generate random in-bounds patches for one RF file.
%
% patch_size  = [Nz_patch, Nx_patch, Ny_patch]
% random_seed = scalar seed for reproducible 1-based starts
% num_patches = number of unique random patches to generate
%
...
% ============================================================
    ...
    if nargin < 4 || isempty(num_patches)
        num_patches = 1;
    end

    num_patches = double(num_patches);

    if numel(num_patches) ~= 1 || num_patches <= 0 || mod(num_patches, 1) ~= 0
        error('num_patches must be a positive integer scalar.');
    end
    ...
    max_unique_starts = prod(max_start);

    if num_patches > max_unique_starts
        error('Requested %d patches, but only %d unique starts exist.', ...
            num_patches, max_unique_starts);
    end
    ...
    starts = zeros(num_patches, 3);
    seen = containers.Map('KeyType', 'char', 'ValueType', 'logical');

    p = 0;
    attempts = 0;
    max_attempts = max(1000, 100 * num_patches);

    while p < num_patches && attempts < max_attempts
        attempts = attempts + 1;

        z0 = randi([1, max_start(1)]);
        x0 = randi([1, max_start(2)]);
        y0 = randi([1, max_start(3)]);
        key = sprintf('%d_%d_%d', z0, x0, y0);
    ...
    if p ~= num_patches
        error('Failed to draw %d unique starts after %d attempts.', ...
            num_patches, attempts);
    end

    patch_list = repmat(struct('z_idx', [], 'x_idx', [], 'y_idx', []), num_patches, 1);
    ...
    for p = 1:num_patches
        ...
        patch_list(p).z_idx = z0:(z0 + Nzp - 1);
        patch_list(p).x_idx = x0:(x0 + Nxp - 1);
        patch_list(p).y_idx = y0:(y0 + Nyp - 1);

        fprintf('[Random patch %03d/%03d] z_idx=%d~%d, x_idx=%d~%d, y_idx=%d~%d\n', ...
            p, num_patches, ...
            patch_list(p).z_idx(1), patch_list(p).z_idx(end), ...
            patch_list(p).x_idx(1), patch_list(p).x_idx(end), ...
            patch_list(p).y_idx(1), patch_list(p).y_idx(end));
        ...
        try
            ...
            switch patch_grid_mode
                ...
                case {'random_single', 'single_random', 'random_v1'}
                    random_seed_i = gen_opts.random_seed + row_idx - 1;
                    patch_list = make_patch_index_list_random_v1( ...
                        scan, patch_size, random_seed_i, num_patches);

                    if isfinite(num_patches) && num_patches ~= length(patch_list)
                        fprintf('[Info] random_single requested manifest num_patches=%g; actual patches=%d.\n', ...
                            num_patches, length(patch_list));
                    end
